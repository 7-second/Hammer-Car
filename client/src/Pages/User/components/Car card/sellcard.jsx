...
>
  <div className="w-50 h-50 object-contain flex flex-col justify-center items-center rounded-full mt-1">
    <img
      src={car?.owner?.profilePicture || "https://via.placeholder.com/150"}
      width={50}
      height={50}
    ...
    <h3 className="text-black font-sans w-52 flex justify-center">
      {car?.owner?.username || "Name of organization"}
    </h3>
  </div>
</Link>
        ...
        <div className={`${variant === "allCars" && "flex md:flex-col"}`}>
          ...
          <div className="mt-[10px]">
            ...
            <div
              className={`${
                variant === "allCars" && "justify-end mb-5 flex-col md:flex-row"
              } flex flex-row md:justify-between  gap-3`}
            >
        ...
        <div
          className={`${
            variant === "own" ? "hidden" : "block"
          } flex justify-between mt-2 gap-x3`}
        >
          <p className="text-xs">ETB {car?.price}</p>
          <button
            type="button"
            onClick={openModal}
