...
const OrgSale = () => {
  ...
  // Fetch cars only once when the component is mounted and when `currentUser` is available.
  useEffect(() => {
    if (!currentUser) {
      navigate('/login');
      return;
    ...
    const getCars = async () => {
      try {
        ...
        const sellCars = response.data.filter(
          (car) => car.carType === "sell" && car.owner._id === currentUser._id // Access car.owner._id
        );
    ...
    // Call the function to fetch the cars data only once
    getCars();
