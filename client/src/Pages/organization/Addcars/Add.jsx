...
const addCarSchema = Z.object({
  carModel: Z.enum(["Corolla", "Vitz", "Yaris", "Hilux", "LandCruiser", "Rav4", "Camry", "HyundaiAccent", "HyundaiSanta", "Dzire", "Swift", "Alto",
    "Sunny", "Patrol", "Navara", "XTrail", "Civic", "Fit", "Lancer", "Pajero", "Outlander", "Jetta", "Passat", "Golf"]).refine(val => val.length > 0, { message: "Model Required" }),
  carType: Z.enum(["rent", "sell"]).refine(val => val.length > 0, { message: 'Type Required' }),
  price: Z.string().min(1, { message: "Price Required" }),
  peopleCapacity: Z.string().min(1, { message: "Capacity in Person Required" }),
  transmission: Z.enum(["Auto", "Manual"]).refine(val => val.length > 0, { message: 'Transmission Required' }),
  carBrand: Z.enum(["Toyota", "Hyundai", "Suzuki", "Nissan", "Volswagen", "Byd", "Lifan", "Jetour", "Honda", "LandRover", "SinoTruck", "Mitsubishi", "Chery"]).refine(val => val.length > 0, { message: "Brand Required" }),
  engine: Z.enum(["Diesel", "Petrol", "Electric", "Gas"]).refine(val => val.length > 0, { message: "Engine Required" }),
  CC: Z.enum(["800", "1000", "1200", "1500", "1800", "2000", "2500", "2800", "3000", "3200", "3500", "4400", "5000", "6000", "6500", "8000"]).refine(val => val.length > 0, { message: "CC Required" }),
...
const AddCarForm = () => {
  ...
  const onSubmit = async (data) => {
    ...
    setIsLoading(true);
    try {
      const response = await axios.post(`${import.meta.env.VITE_API_BASE_URL}car/add-car`, formData);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col -mt-4 gap-3 p-4'>
      ...
      <div className='grid md:grid-cols-2 gap-3 md:gap-x-6 gap-y-8 p-2'>
        ...
        <div className='flex flex-col gap-1'>
          ...
          <select {...register('carBrand')} className="w-full bg-[#f1f1f1] p-2 outline-none text-sm rounded-md">
        ...
        <div className='flex flex-col gap-1'>
          ...
          <select {...register('carModel')} className="w-full bg-[#f1f1f1] p-2 outline-none text-sm rounded-md">
            <option value="">select</option>
        ...
        <div className='flex flex-col gap-1'>
          ...
          <select {...register('engine')} className="w-full bg-[#f1f1f1] p-2 outline-none text-sm rounded-md">
            <option value="">select</option>
        ...
        <div className='flex flex-col gap-1'>
          ...
          <select {...register('CC')} className="w-full bg-[#f1f1f1] p-2 outline-none text-sm rounded-md">
            <option value="">select</option>
          ...
          {errors.Cc && <p className='text-red-500 text-xs'>{errors.Cc.message}</p>}
        </div>

        <div className='flex flex-col gap-1'>
          <label className='text-xs font-bold'>Car For</label>
          <select {...register('carType')} className="w-full bg-[#f1f1f1] p-2 outline-none text-sm rounded-md">
            <option value="">select</option>
        ...
        <div className='flex flex-col gap-1'>
          ...
          <input {...register('price')} type="number" className='bg-[#f1f1f1] p-2 outline-none text-sm rounded-md' placeholder='Price in ETH (Birr)' />
          {errors.price && <p className='text-red-500 text-xs'>{errors.price.message}</p>}
        </div>

       <div className='flex flex-col gap-1'>
          <label className='text-xs font-bold'>Year</label>
          <input {...register('year')} type="number" className='bg-[#f1f1f1] p-2 outline-none text-sm rounded-md' placeholder='Price in ETH (Birr)' />
          {errors.price && <p className='text-red-500 text-xs'>{errors.price.message}</p>}
        </div>

        <div className='flex flex-col gap-1'>
          <label className='text-xs font-bold'>People Capacity</label>
          <input {...register('peopleCapacity')} type="number" className='bg-[#f1f1f1] p-2 outline-none text-sm rounded-md' placeholder="Capacity in persons" />
        ...
        <div className='flex flex-col gap-1'>
          ...
          <select {...register('transmission')} className="w-full bg-[#f1f1f1] p-2 outline-none text-sm rounded-md">
            <option value="">select</option>
        ...
        <div className='flex flex-col gap-1'>
          ...
          <input {...register('location')} placeholder="Your City" className='bg-[#f1f1f1] p-2 outline-none text-sm rounded-md' />
          {errors.location && <p className='text-red-500 text-xs'>{errors.location.message}</p>}
        </div>

        <div className='flex flex-col gap-1'>
          <label className='text-xs font-bold'>Fuel Capacity</label>
          <input {...register('fuelCapacity')} type="number" placeholder="Fuel Capacity in liters" className='bg-[#f1f1f1] p-2 outline-none text-sm rounded-md' />
        ...
        <div className='flex flex-col gap-1'>
          ...
          <input {...register('description')} placeholder="Enter a short Description" className='bg-[#f1f1f1] p-2 outline-none text-sm rounded-md' />
      ...
      <div className="flex flex-col gap-1 w-full">
        ...
        <label htmlFor="images" className="flex flex-col gap-1 justify-center items-center w-full h-52 border-2 border-dashed rounded-sm overflow-auto p-5">
        ...
        <input type="file" id="images" accept="image/*" multiple hidden onChange={handleFileChange} />
      </div>
      ...
      <div className='mt-6 flex flex-col gap-2 items-center'>
        <button
          disabled={isLoading}
          type="submit"
          className={`flex items-center justify-center ${isLoading ? "bg-gray-500" : "bg-blue-500"} py-2 px-5 hover:bg-blue-700 text-xs font-medium text-white rounded-md min-w-32 max-w-32`}>
