...
const CarDetail = ({ car, variant, isOpen, setIsOpen }) => {

    // Get user
    const currentUser = User()

    // Is current user rented this car
    const filterRentedCar = currentUser?.rent?.filter((rented) => rented?.car?._id === car?._id)

    // Is this car belongs to current user
    const filterOwnCar = currentUser?.cars?.filter((cars) => cars?._id === car?._id)
